...
int ax25getFrame(uint8_t *raw,AX25Ctx *ctx, ax25frame *pkg)
{
    int i, j, c = 0;
    uint8_t data = 0;
    ctx->crc_out = CRC_CCIT_INIT_VAL;
    int raw_count=0;
    uint8_t *rawBegin = raw;
    //tx_putchar(HDLC_FLAG);
    *raw++ = HDLC_FLAG;
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        for (j = 0; j < 6; j++)
        {
            ...
            // putchar(data);
            data <<= 1;
            raw=ax25_putRaw(raw,ctx, data);
            c++;
        }
        raw=ax25_putRaw(raw,ctx, (uint8_t)pkg->header[i].ssid);
        if (pkg->header[i].ssid & 0x01)
            break;
    }

    raw=ax25_putRaw(raw,ctx, AX25_CTRL_UI);      // Control field - 0x03 is APRS UI-frame
    raw=ax25_putRaw(raw,ctx, AX25_PID_NOLAYER3); // Protocol ID - 0xF0 is no layer 3
                                          // ax25sendString(&pkg->data[0]);

    for (i = 0; i < strlen(pkg->data); i++)
    {
        raw=ax25_putRaw(raw,ctx, (uint8_t)pkg->data[i]);
    }

    uint8_t crcl = (ctx->crc_out & 0xff) ^ 0xff;
    uint8_t crch = (ctx->crc_out >> 8) ^ 0xff;
    raw=ax25_putRaw(raw,ctx, crcl);
    raw=ax25_putRaw(raw,ctx, crch);

    *raw++ = HDLC_FLAG;
    int sz=(int)(raw-rawBegin);
    return sz;
}